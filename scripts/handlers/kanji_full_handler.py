"""
Handler for Kanji Full dataset - comprehensive kanji character recognition.

Kanji Full dataset characteristics:
- Complete kanji character coverage
- Multiple writing styles and variations
- High-resolution character images
- Character metadata and classification
- Suitable for character recognition and classification tasks

Metadata: character code, Unicode mapping, writing style variations
"""
...
class KanjiFulHandler(DatasetFormatHandler):
    """Handler for Kanji Full character dataset."""
    ...
    def download(self, output_dir: Path, force: bool = False) -> Path:
        ...
        """
        logger.info("Kanji Full download handled by download_research_datasets.py")
        logger.info("Dataset is downloaded from Hugging Face: epts/kanji-full")
        dataset_path = output_dir / "kanji_full"
        dataset_path.mkdir(parents=True, exist_ok=True)
        return dataset_path

    def extract(self, archive_path: Path, output_dir: Path) -> Path:
        """
        Extract Kanji Full archive.
        ...
        dataset_dir = output_dir / "kanji_full"
        ...
        try:
            ...
            logger.info("✓ Extracted Kanji Full")
            return dataset_dir
        except Exception as e:
            logger.error(f"Failed to extract Kanji Full: {e}")
            return None
    ...
    def parse_record(self, record: Any) -> Optional[Dict[str, Any]]:
        ...
        Args:
            record: Dict with image, label, character_code, unicode
                ...
                if image is not None and label is not None:
                    return {
                        ...
                        "metadata": {
                            ...
                            "writing_style": record.get("writing_style", "standard"),
    ...
    def iter_records(self, dataset_path: Path) -> List[Any]:
        """
        Iterate over all records in Kanji Full.
        ...
        try:
            # Check for HF cache structure (epts___kanji-full/default/0.0.0/.../)
                                    ...
                                    records.append(
                                        {
                                            ...
                                            "writing_style": row.get("writing_style", "standard"),
                                        }
                                    )
    ...
    def get_manifest(self, dataset_path: Path) -> Optional[DatasetManifest]:
        ...
        try:
            ...
            manifest = DatasetManifest(
                ...
                source_url="https://huggingface.co/datasets/epts/kanji-full",
            )
    ...
    def get_dataset_manifest(self) -> Optional[DatasetManifest]:
        ...
        return DatasetManifest(
            name="Kanji Full",
            description="Comprehensive kanji character dataset from Hugging Face (epts/kanji-full)",
            num_samples=3000,  # Approximate
            num_classes=3036,  # Standard kanji count
            tags=["kanji", "japanese", "character-recognition", "huggingface"],
            source_url="https://huggingface.co/datasets/epts/kanji-full",
        )
